...
if (!function_exists('cs_blog_shortcode')) {
	function cs_blog_shortcode( $atts ) {
		...
		if ( $cs_blog_view == 'blog-3-column'  || $cs_blog_view == 'blog-masonry' ) {
			$outerDivStart	= '<div class="mas-isotope-'.$randomId.'" '.$CustomId.'>';
			$outerDivEnd	= '</div>';
 		}
			...
			$postCounter	= 0;
					  ...
					  $last_child = ( $post_count == $postCounter ) ? 'last_child' : '' ;
							if ( $cs_blog_view == 'blog-small' ) {
								$blogObject->cs_small_view( $cs_blog_description , $cs_blog_excerpt,$cs_blog_cat );
							...
							} else if ( $cs_blog_view == 'blog-masonry' ) {
								$blogObject->cs_masonry_view( $cs_blog_description , $cs_blog_excerpt,$cs_blog_cat);
					  ...
					  echo cs_allow_special_char( $outerDivEnd );
						 ...
						 //==Pagination End	
						 if ( $cs_blog_view == 'blog-masonry' ) {
                         ?>
                                 <script>
							 jQuery(document).ready(function($) {
							  var container = jQuery(".mas-isotope-<?php echo cs_allow_special_char($randomId);?>").imagesLoaded(function() {
								container.isotope()
							  });
							  jQuery(window).resize(function() {
								setTimeout(function() {
								  jQuery(".mas-isotope-<?php echo cs_allow_special_char($randomId);?>").isotope()
								}, 600)
							  });
							 });
						</script>
