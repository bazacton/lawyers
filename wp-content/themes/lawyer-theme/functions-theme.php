...
if (!function_exists('cs_flex_slider')) {
    ...
    function cs_flex_slider($width, $height, $slider_id) {
        ...
        if ($cs_theme_options['flex_auto_play'] == 'on') {
            $auto_play = 'true';
        } else if ($cs_theme_options['flex_auto_play'] == '') {
            $auto_play = 'false';
        }
...
if (!function_exists('cs_pagination')) {
    ...
    function cs_pagination($total_records, $per_page, $qrystr = '', $show_pagination = 'Show Pagination') {
        ...
        if ($show_pagination <> 'Show Pagination') {
            ...
            if ($page_id_all > 1)
                $html .= "<li class='pgprev'><a href='?page_id_all=" . ($page_id_all - 1) . "$qrystr' >" . __('<i class="icon-long-arrow-left"></i> Previous', 'Lawyer') . "</a></li>";
            ...
            if ($page_id_all < $total_records / $per_page)
                $html .= "<li class='pgnext'><a class='icon' href='?page_id_all=" . ($page_id_all + 1) . "$qrystr' >" . __('Next <i class="icon-long-arrow-right"></i>', 'Lawyer') . "</a></li>";
            $html .= "</ul></nav></div>";
            return $html;
        }
    }
...
if (!function_exists('cs_social_network')) {
    ...
    function cs_social_network($icon_type = '', $tooltip = '') {
        ...
        if (isset($cs_theme_options['social_net_url']) and count($cs_theme_options['social_net_url']) > 0) {
            ...
            $i = 0;
            echo '<ul >';
            if (is_array($cs_theme_options['social_net_url'])) {
    ...
    function cs_social_network_widget($icon_type = '', $tooltip = '') {
        ...
        if (isset($cs_theme_options['social_net_url']) and count($cs_theme_options['social_net_url']) > 0) {
            ...
            foreach ($cs_theme_options['social_net_url'] as $val) {
                ...
                <?php if ($val != '') { ?><a class="cs-colrhvr" title="" href="<?php echo esc_url($val); ?>" data-original-title="<?php echo esc_attr($cs_theme_options['social_net_tooltip'][$i]); ?>" data-placement="top" <?php echo balanceTags($tooltip_data, false); ?> target="_blank"><?php if ($cs_theme_options['social_net_awesome'][$i] <> '' && isset($cs_theme_options['social_net_awesome'][$i])) { ?> 


                            <i class="<?php echo esc_attr($cs_theme_options['social_net_awesome'][$i]); ?>"></i>
...
if (!function_exists('cs_bg_image')) {
    ...
    function cs_bg_image() {
        ...
        $cs_bg_image = isset($cs_bg_image) ? $cs_bg_image : '';
        if ($cs_theme_options['cs_custom_bgimage'] == "") {
            if (isset($cs_theme_options['cs_bg_image']) && $cs_theme_options['cs_bg_image'] != ''
...
//primary widget
register_sidebar(array(
    'name' => __('Primary Sidebar', 'Lawyer'),
    'id' => 'sidebar-1',
    'description' => __('Main sidebar that appears on the right.', 'Lawyer'),
...
register_sidebar(array(
    ...
    'before_widget' => '<aside class="col-md-3 widget %2$s">',
    'after_widget' => '</aside>',
    'before_title' => '<div class="widget-section-title"><h2>',
    'after_title' => '</h2></div>'
));
        ...
        switch ($comment->comment_type) :
            ...
            case '' :
                ...
                <li class="comment byuser comment-author-admin bypostauthor even thread-even depth-1" <?php //comment_class();         ?> id="li-comment-<?php comment_ID(); ?>">
                    <div class="thumblist" id="comment-<?php comment_ID(); ?>">
                        <ul>
                            <li>
                                <header>
                                    <figure class="blog-comment"><a><?php echo get_avatar($comment, 54); ?></a></figure>
                                    <h6><a href="<?php get_comment_author_url(); ?>"><?php comment_author(); ?></a></h6>
                                    <?php printf(__('<span>%1$s</span>', 'Lawyer'), get_comment_date() . ', ' . get_comment_time()); ?>
                                </header>
                                <div class="text">
                                    <?php if ($comment->comment_approved == '0') : ?>
                                        <p><div class="comment-awaiting-moderation colr"><?php _e('Your comment is awaiting moderation.', 'Lawyer'); ?></div></p>
                                    <?php endif; ?>
                ...
                <li class="post pingback">
                    <p><?php comment_author_link(); ?><?php edit_comment_link(__('(Edit)', 'Lawyer'), ' '); ?></p>
                    <?php
                    break;
            endswitch;
        }
                                        ...
                                        <div class="under-wrapp">
                                            ...
                                            <div class="cons-text-wrapp">
                                                ...
                                                if ($cs_uc_options['cs_maintenance_text']) {
                                                    ...
                                                    ?>
                                                    <h1><?php _e('Sorry, We are down for maintenance', 'Lawyer'); ?></h1>
                                                    <p><?php _e('We\'re currently under maintenance, if all goes as planned we\'ll be back in', 'Lawyer'); ?></p>
                                                    <?php
                                                }
                                                ?>
    ...
    if (!function_exists('cs_page_404')) {
        ...
        <html <?php language_attributes(); ?>>
            <head>
                ...
                <link rel="stylesheet" href="<?php echo get_template_directory_uri() . '/assets/css/themetypo.css'; ?>" />
                                            ...
                                            <div class="col-md-12">
                                                <div class="page-not-found">
                                                    ...
                                                    <div class="cs-section-title">
                                                        <h2><?php _e('<span class="cs-color">404</span> ERROR', 'Lawyer'); ?></h2>
                                                    </div>
                                                    <!-- Header End -->
                                                    <div class="cs-content404">
                                                        <aside class="cs-icon"> <i class="icon-warning"></i> </aside>
                                                        <div class="desc">
                                                            <p><?php _e('The Page you Were Looking for Could not be found.', 'Lawyer'); ?></p>
                                                        </div>
                                                    </div>
                                                    <?php get_search_form(); ?>
                                                </div>
...
if (!function_exists('cs_breadcrumbs')) {
    ...
    function cs_breadcrumbs() {
        ...
        /* === END OF OPTIONS === */
        $current_page = __('Current Page', 'Lawyer');
        $homeLink = home_url() . '/';
        $linkBefore = '<li>';
        $linkAfter = '</li>';
        $linkAttr = '';
        ...
        if (is_home() || is_front_page()) {
            ...
            if ($showOnHome == "1")
                echo '<ul>' . $before . '<a href="' . $homeLink . '">' . $text['home'] . '</a>' . $after . '</ul></div>';
        } else {
...
if (!function_exists('cs_footer_logo')) {
    ...
    function cs_footer_logo() {
        global $cs_theme_options;
        $logo = isset($cs_theme_options['cs_footer_logo']) && $cs_theme_options['cs_footer_logo'] ? $cs_theme_options['cs_footer_logo'] : get_template_directory_uri() . '/assets/images/footer-logo.png';
