...
function cs_meta_post_add() {
    add_meta_box('cs_meta_post', __('Post Options', 'Lawyer'), 'cs_meta_post', 'post', 'normal', 'high');
}
                    ...
                    <div class="tabs vertical">
                        <nav class="admin-navigtion">
                            <ul id="myTab" class="nav nav-tabs">
                                <li class="active"><a href="#tab-general-settings" data-toggle="tab"><i class="icon-toggle-right"></i><?php _e('General Settings', 'Lawyer'); ?></a></li>
                                <li><a href="#tab-subheader-options" data-toggle="tab"><i class="icon-list-alt"></i> <?php _e('Sub Header Options', 'Lawyer'); ?> </a></li>
                                ...
                                <?php } ?>
                                <li><a href="#tab-post-options" data-toggle="tab"><i class="icon-list-alt"></i> <?php _e('Post Settings', 'Lawyer'); ?> </a></li>
                            ...
                            <?php if ($cs_builtin_seo_fields == 'on') { ?>
                                <div id="tab-seo-advance-settings" class="tab-pane fade">
                                    <div class="theme-help">
                                        <h4 style="padding-bottom:0px;"><?php _e('Seo Options', 'Lawyer'); ?></h4>
                                        <div class="clear"></div>
                                    </div>
        ...
        if (isset($_POST['gallery_meta_form']) and $_POST['gallery_meta_form'] == 1) {
            ...
            if (isset($_POST['path'])) {
                foreach ($_POST['path'] as $count) {
                    if (empty($_POST['path'][$cs_counter])) {
                        $_POST['path'][$cs_counter] = "";
                    }
                    if (empty($_POST['title'][$cs_counter])) {
                        $_POST['title'][$cs_counter] = "";
                    }
                    if (empty($_POST['use_image_as'][$cs_counter])) {
                        $_POST['use_image_as'][$cs_counter] = "";
                    }
                    if (empty($_POST['video_code'][$cs_counter])) {
                        $_POST['video_code'][$cs_counter] = "";
                    }
                    if (empty($_POST['link_url'][$cs_counter])) {
                        $_POST['link_url'][$cs_counter] = "";
                    }
        ...
        if (isset($_POST['gallery_slider_meta_form']) and $_POST['gallery_slider_meta_form'] == 1) {
            ...
            if (isset($_POST['cs_slider_path'])) {
                ...
                foreach ($_POST['cs_slider_path'] as $count) {
                    if (empty($_POST['cs_slider_path'][$cs_counter])) {
                        $_POST['cs_slider_path'][$cs_counter] = "";
                    }
                    if (empty($_POST['cs_slider_title'][$cs_counter])) {
                        $_POST['cs_slider_title'][$cs_counter] = "";
                    }
                    if (empty($_POST['slider_use_image_as'][$cs_counter])) {
                        $_POST['slider_use_image_as'][$cs_counter] = "";
                    }
                    if (empty($_POST['slider_video_code'][$cs_counter])) {
                        $_POST['slider_video_code'][$cs_counter] = "";
                    }
                    if (empty($_POST['cs_slider_link'][$cs_counter])) {
                        $_POST['cs_slider_link'][$cs_counter] = "";
                    }
...
if (!function_exists('cs_blog_post_general_options')) {
    ...
    function cs_blog_post_general_options() {
        global $cs_xmlObject;
        if (empty($cs_xmlObject->post_thumb_view))
            $post_thumb_view = "";
        else
            $post_thumb_view = $cs_xmlObject->post_thumb_view;
        if (empty($cs_xmlObject->post_thumb_audio))
            $post_thumb_audio = "";
        else
            $post_thumb_audio = $cs_xmlObject->post_thumb_audio;
        if (empty($cs_xmlObject->post_thumb_video))
            $post_thumb_video = "";
        else
            $post_thumb_video = $cs_xmlObject->post_thumb_video;
        if (empty($cs_xmlObject->post_thumb_slider))
            $post_thumb_slider = "";
        else
            $post_thumb_slider = $cs_xmlObject->post_thumb_slider;
        if (empty($cs_xmlObject->post_thumb_slider_type))
            $post_thumb_slider_type = "";
        else
            $post_thumb_slider_type = $cs_xmlObject->post_thumb_slider_type;
        if (empty($cs_xmlObject->inside_post_thumb_view))
            $inside_post_thumb_view = "";
        else
            $inside_post_thumb_view = $cs_xmlObject->inside_post_thumb_view;
        if (empty($cs_xmlObject->inside_post_thumb_audio))
            $inside_post_thumb_audio = "";
        else
            $inside_post_thumb_audio = $cs_xmlObject->inside_post_thumb_audio;
        if (empty($cs_xmlObject->inside_post_thumb_video))
            $inside_post_thumb_video = "";
        else
            $inside_post_thumb_video = $cs_xmlObject->inside_post_thumb_video;
        if (empty($cs_xmlObject->inside_post_thumb_slider))
            $inside_post_thumb_slider = "";
        else
            $inside_post_thumb_slider = $cs_xmlObject->inside_post_thumb_slider;
        if (empty($cs_xmlObject->inside_post_thumb_slider_type))
            $inside_post_thumb_slider_type = "";
        else
            $inside_post_thumb_slider_type = $cs_xmlObject->inside_post_thumb_slider_type;
        ?>
        <ul class="form-elements noborder">
            <li class="to-label"><label><?php _e('Thumbnail View', 'Lawyer'); ?></label></li>
            <li class="to-field">
                <div class="input-sec">
                    <div class="select-style">
                        <select name="post_thumb_view" class="dropdown" onchange="javascript:new_toggle(this.value)">
                            <option <?php if ($post_thumb_view == "Single Image") echo "selected"; ?> ><?php _e('Single Image', 'Lawyer'); ?></option>
                            <option <?php if ($post_thumb_view == "Slider") echo "selected"; ?> ><?php _e('Slider', 'Lawyer'); ?></option>
                        </select>
                    </div>
                </div>
                <div class="left-info">
                    ...
                    if ($post_thumb_view == "Single Image" or $post_thumb_view == "")
                        ...
                        echo 'none';
                    ?>"><?php _e('Use Featured Image as Thumbnail', 'Lawyer'); ?></p>
                </div>
            </li>
            ...
            if ($post_thumb_view == "Audio")
                ...
                echo 'none';
            ?>" >
                <li class="to-label"><label><?php _e('Audio Url', 'Lawyer'); ?></label></li>
                <li class="to-field">
                    <div class="input-sec">
                    ...
                    <div class="left-info">
                        <p><?php _e('Enter Specific Audio Url (Youtube, Vimeo and all others wordpress supported)', 'Lawyer'); ?></p>
                    </div>
                </li>
            </ul>
            ...
            if ($post_thumb_view == "Video")
                ...
                echo 'none';
            ?>" >
                <li class="to-label"><label><?php _e('Thumbnail Video Url', 'Lawyer'); ?></label></li>
                <li class="to-field">
                    <div class="input-sec">
                    ...
                    <div class="left-info">
                        <p><?php _e('Enter Specific Video Url (Youtube, Vimeo and all others wordpress supported) OR you can select it from your media library', 'Lawyer'); ?></p>
                    </div>
                </li>
            </ul>
        </ul>
        <div class="noborder post_slider" id="post_thumb_slider" style="display:<?php
        ...
        <ul class="form-elements noborder">
            <li class="to-label"><label><?php _e('Inside Post Thumbnail View', 'Lawyer'); ?></label></li>
            <li class="to-field">
                <div class="input-sec">
                    <div class="select-style">
                        <select name="inside_post_thumb_view" class="dropdown" onchange="javascript:new_toggle_inside_post(this.value)">
                            <option <?php if ($inside_post_thumb_view == "Single Image") echo "selected"; ?> ><?php _e('Single Image', 'Lawyer'); ?></option>
                            <option <?php if ($inside_post_thumb_view == "Audio") echo "selected"; ?> ><?php _e('Audio', 'Lawyer'); ?></option>
                            <option <?php if ($inside_post_thumb_view == "Video") echo "selected"; ?> value="Video"><?php _e('Video/Soundcloud', 'Lawyer'); ?></option>
                            <option <?php if ($inside_post_thumb_view == "Slider") echo "selected"; ?> ><?php _e('Slider', 'Lawyer'); ?></option>
                        </select>
                    </div>
                </div>
                <div class="left-info">
        ...
        <div class="" id="inside_post_thumb_slider" style="display:<?php
