...
if (!class_exists('TGM_Plugin_Activation')) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function init() {
            ...
            // Load class strings.
            $this->strings = array(
                'page_title' => __('Install Required Plugins', 'Lawyer'),
                'menu_title' => __('Install Plugins', 'Lawyer'),
                'installing' => __('Installing Plugin: %s', 'Lawyer'),
                'oops' => __('Something went wrong.', 'Lawyer'),
                ...
                'notice_cannot_install' => _n_noop(
                        'Sorry, but you do not have the correct permissions to install the %s plugin.', 'Sorry, but you do not have the correct permissions to install the %s plugins.', 'tgmpa'
                ),
                ...
                'notice_cannot_activate' => _n_noop(
                        'Sorry, but you do not have the correct permissions to activate the %s plugin.', 'Sorry, but you do not have the correct permissions to activate the %s plugins.', 'tgmpa'
                ),
                ...
                'notice_cannot_update' => _n_noop(
                        'Sorry, but you do not have the correct permissions to update the %s plugin.', 'Sorry, but you do not have the correct permissions to update the %s plugins.', 'tgmpa'
                ),
                ...
                'return' => __('Return to Required Plugins Installer', 'Lawyer'),
                'dashboard' => __('Return to the dashboard', 'Lawyer'),
                'plugin_activated' => __('Plugin activated successfully.', 'Lawyer'),
                'activated_successfully' => __('The following plugin was activated successfully:', 'Lawyer'),
                'complete' => __('All plugins installed and activated successfully. %1$s', 'Lawyer'),
                'dismiss' => __('Dismiss this notice', 'Lawyer'),
                'contact_admin' => __('Please contact the administrator of this site for help.', 'Lawyer'),
            );
        ...
        protected function do_plugin_install() {
            ...
            if (isset($this->plugins[$slug], $_GET['tgmpa-install']) && 'install-plugin' === $_GET['tgmpa-install']) {
                ...
                if (true === $complete) {
                    echo '<p>', sprintf(esc_html($this->strings['complete']), '<a href="' . esc_url(self_admin_url()) . '">' . esc_html__('Return to the Dashboard', 'Lawyer') . '</a>'), '</p>';
            ...
            if (!empty($desired_slug)) {
                ...
                if (!empty($subdir_name) && $subdir_name !== $desired_slug) {
                    ...
                    if (true === $GLOBALS['wp_filesystem']->move($from, $to)) {
                        ...
                        return new WP_Error('rename_failed', esc_html__('The remote plugin package is does not contain a folder with the desired slug and renaming did not work.', 'Lawyer') . ' ' . esc_html__('Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'Lawyer'), array('found' => $subdir_name, 'expected' => $desired_slug));
                    }
                } elseif (empty($subdir_name)) {
                    return new WP_Error('packaged_wrong', esc_html__('The remote plugin package consists of more than one file, but the files are not packaged in a folder.', 'Lawyer') . ' ' . esc_html__('Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'Lawyer'), array('found' => $subdir_name, 'expected' => $desired_slug));
                }
            }
        ...
        protected function activate_single_plugin($file_path, $slug, $automatic = false) {
            ...
            if (!is_plugin_active($file_path)) {
                ...
                sprintf(
                        esc_html__('No action taken. Plugin %1$s was already active.', 'Lawyer'), '<strong>' . esc_html($this->plugins[$slug]['name']) . '</strong>'
                ),
                '</p></div>';
            }
        ...
        public function notices() {
            ...
            if (!empty($message)) {
                ...
                foreach ($message as $type => $plugin_group) {
                    ...
                    $imploded = empty($linked_plugins) ? $last_plugin : ( implode(', ', $linked_plugins) . ' ' . esc_html_x('and', 'plugin A *and* plugin B', 'Lawyer') . ' ' . $last_plugin );
                ...
                $show_install_link = $install_link ? '<a href="' . esc_url($this->get_tgmpa_url()) . '">' . translate_nooped_plural($this->strings['install_link'], $install_link_count, 'Lawyer') . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . esc_url($this->get_tgmpa_url()) . '">' . translate_nooped_plural($this->strings['activate_link'], $activate_link_count, 'Lawyer') . '</a>' : '';
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ($this->tgmpa->plugins as $slug => $plugin) {
                ...
                if ('repo' !== $plugin['source'] && preg_match(TGM_Plugin_Activation::WP_REPO_REGEX, $plugin['source']) !== 1) {
                    if (preg_match(TGM_Plugin_Activation::IS_URL_REGEX, $plugin['source'])) {
                        ...
                        $table_data[$i]['source'] = __('External Source', 'Lawyer');
                    } else {
                        // The plugin is pre-packaged with the theme.
                        $table_data[$i]['source'] = __('Pre-Packaged', 'Lawyer');
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __('WordPress Repository', 'Lawyer');
                }

                if (isset($plugin['required']) && $plugin['required']) {
                    $table_data[$i]['type'] = __('Required', 'Lawyer');
                } else {
                    $table_data[$i]['type'] = __('Recommended', 'Lawyer');
                }

                if (!isset($installed_plugins[$plugin['file_path']])) {
                    $table_data[$i]['status'] = sprintf('%1$s', __('Not Installed', 'Lawyer'));
                } elseif (is_plugin_inactive($plugin['file_path'])) {
                    $table_data[$i]['status'] = sprintf('%1$s', __('Installed But Not Activated', 'Lawyer'));
                }
        ...
        public function column_plugin($item) {
            ...
            if (!is_plugin_active($item['file_path']) && !isset($installed_plugins[$item['file_path']])) {
                ...
                $actions = array(
                    'install' => sprintf(
                            '<a href="%1$s">' . esc_html_x('Install %2$s', '%2$s = plugin name in screen reader markup', 'Lawyer') . '</a>', esc_url($install_nonce_url), '<span class="screen-reader-text">' . esc_html($item['sanitized_plugin']) . '</span>'
                    ),
                );
            }
            ...
            elseif (is_plugin_inactive($item['file_path'])) {
                ...
                $actions = array(
                    'activate' => sprintf(
                            '<a href="%1$s">' . esc_html_x('Activate %2$s', '%2$s = plugin name in screen reader markup', 'Lawyer') . '</a>', esc_url($activate_url), '<span class="screen-reader-text">' . esc_html($item['sanitized_plugin']) . '</span>'
                    ),
                );
            }
        ...
        public function no_items() {
            ...
            printf(wp_kses_post(__('No plugins to install or activate. <a href="%1$s">Return to the Dashboard</a>', 'Lawyer')), esc_url(self_admin_url()));
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        }
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __('Plugin', 'Lawyer'),
                'source' => __('Source', 'Lawyer'),
                'type' => __('Type', 'Lawyer'),
                'status' => __('Status', 'Lawyer'),
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install' => __('Install', 'Lawyer'),
                'tgmpa-bulk-activate' => __('Activate', 'Lawyer'),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ('tgmpa-bulk-install' === $this->current_action()) {
                ...
                if (empty($plugins_to_install)) {
                    echo '<div id="message" class="error"><p>', esc_html__('No plugins are available to be installed at this time.', 'Lawyer'), '</p></div>';
                    return false;
                }
            ...
            if ('tgmpa-bulk-activate' === $this->current_action()) {
                ...
                if (empty($plugins_to_activate)) {
                    echo '<div id="message" class="error"><p>', esc_html__('No plugins are available to be activated at this time.', 'Lawyer'), '</p></div>';
                    return false;
                }
                ...
                if (is_wp_error($activate)) {
                    ...
                    $imploded = empty($plugin_names) ? $last_plugin : ( implode(', ', $plugin_names) . ' ' . esc_html_x('and', 'plugin A *and* plugin B', 'Lawyer') . ' ' . $last_plugin );

                    printf(
                            '<div id="message" class="updated"><p>%1$s %2$s.</p></div>', esc_html(_n('The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'Lawyer')), $imploded // WPCS: xss ok
                    );
            ...
            if (!class_exists('TGM_Bulk_Installer')) {
                ...
                class TGM_Bulk_Installer extends WP_Upgrader {
                    ...
                    public function install_strings() {
                        ...
                        $this->strings['no_package'] = __('Install package not available.', 'Lawyer');
                        $this->strings['downloading_package'] = __('Downloading install package from <span class="code">%s</span>&#8230;', 'Lawyer');
                        $this->strings['unpack_package'] = __('Unpacking the package&#8230;', 'Lawyer');
                        $this->strings['installing_package'] = __('Installing the plugin&#8230;', 'Lawyer');
                        $this->strings['process_failed'] = __('Plugin install failed.', 'Lawyer');
                        $this->strings['process_success'] = __('Plugin installed successfully.', 'Lawyer');
                    }
                    ...
                    public function activate_strings() {
                        ...
                        $this->strings['activation_failed'] = __('Plugin activation failed.', 'Lawyer');
                        $this->strings['activation_success'] = __('Plugin activated successfully.', 'Lawyer');
                    }
            ...
            if (!class_exists('TGM_Bulk_Installer_Skin')) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
                    ...
                    public function add_strings() {
                        ...
                        $this->upgrader->strings['skin_update_failed_error'] = __('An error occurred while installing %1$s: <strong>%2$s</strong>.', 'Lawyer');
                        $this->upgrader->strings['skin_update_failed'] = __('The installation of %1$s failed.', 'Lawyer');

                        // Automatic activation strings.
                        if ($this->tgmpa->is_automatic) {
                            $this->upgrader->strings['skin_upgrade_start'] = __('The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'Lawyer');
                            $this->upgrader->strings['skin_update_successful'] = __('%1$s installed and activated successfully.', 'Lawyer') . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__('Show Details', 'Lawyer') . '</span><span class="hidden">' . esc_html__('Hide Details', 'Lawyer') . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end'] = __('All installations and activations have been completed.', 'Lawyer');
                            $this->upgrader->strings['skin_before_update_header'] = __('Installing and Activating Plugin %1$s (%2$d/%3$d)', 'Lawyer');
                        }
                        // Default installation strings.
                        else {
                            $this->upgrader->strings['skin_upgrade_start'] = __('The installation process is starting. This process may take a while on some hosts, so please be patient.', 'Lawyer');
                            $this->upgrader->strings['skin_update_successful'] = esc_html__('%1$s installed successfully.', 'Lawyer') . ' <a href="#" class="hide-if-no-js" onclick="%2$s"><span>' . esc_html__('Show Details', 'Lawyer') . '</span><span class="hidden">' . esc_html__('Hide Details', 'Lawyer') . '</span>.</a>';
                            $this->upgrader->strings['skin_upgrade_end'] = __('All installations have been completed.', 'Lawyer');
                            $this->upgrader->strings['skin_before_update_header'] = __('Installing Plugin %1$s (%2$d/%3$d)', 'Lawyer');
                        }
                    }
                    ...
                    public function before($title = '') {
                        ...
                        printf(
                                '<h4>' . wp_kses_post($this->upgrader->strings['skin_before_update_header']) . ' <img alt="" src="%4$s" class="hidden waiting-%5$s" style="vertical-align:middle;" /></h4>', esc_html($this->plugin_names[$this->i]), absint($this->upgrader->update_current), absint($this->upgrader->update_count), esc_url(admin_url('images/wpspin_light.gif')), esc_attr($this->upgrader->update_current)
                        );
                    ...
                    public function bulk_footer() {
                        ...
                        if (true === $complete) {
                            echo '<p>', sprintf(esc_html($this->tgmpa->strings['complete']), '<a href="' . esc_url(self_admin_url()) . '">' . esc_html__('Return to the Dashboard', 'Lawyer') . '</a>'), '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
                    }
