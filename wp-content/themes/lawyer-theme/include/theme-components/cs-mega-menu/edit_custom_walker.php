...
class Walker_Nav_Menu_Edit_Custom extends Walker_Nav_Menu {
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0) {
        ...
        if (!empty($item->_invalid)) {
            ...
            $title = sprintf(__('%s (Invalid)', 'Lawyer'), $item->title);
        } elseif (isset($item->post_status) && 'draft' == $item->post_status) {
            $classes[] = 'pending';
            /* translators: %s: title of menu item in draft status */
            $title = sprintf(__('%s (Pending)', 'Lawyer'), $item->title);
        }
            ...
            <dl class="menu-item-bar">
                ...
                <span class="item-controls">
                    ...
                    <span class="item-order hide-if-js">
                        ...
                        ?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up', 'Lawyer'); ?>">&#8593;</abbr></a>
                        |
                        <a href="<?php
                        echo wp_nonce_url(
                                add_query_arg(
                        ...
                        ?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down', 'Lawyer'); ?>">&#8595;</abbr></a>
                    </span>
                    <a class="item-edit" id="edit-<?php echo intval($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item', 'Lawyer'); ?>" href="<?php
                    echo ( isset($_GET['edit-menu-item']) && $item_id == $_GET['edit-menu-item'] ) ? admin_url('nav-menus.php') : add_query_arg('edit-menu-item', $item_id, remove_query_arg($removed_args, admin_url('nav-menus.php#menu-item-settings-' . $item_id)));
                    ?>"><?php _e('Edit Menu Item', 'Lawyer'); ?></a>
                </span>
                </dt>
            </dl>
            ...
            <div class="menu-item-settings" id="menu-item-settings-<?php echo intval($item_id); ?>">
                ...
                <p class="description description-thin">
                    <label for="edit-menu-item-title-<?php echo intval($item_id); ?>">
                        <?php _e('Navigation Label', 'Lawyer'); ?><br />
                ...
                <p class="description description-thin">
                    <label for="edit-menu-item-attr-title-<?php echo intval($item_id); ?>">
                        <?php _e('Title Attribute', 'Lawyer'); ?><br />
                ...
                <p class="field-link-target description">
                    <label for="edit-menu-item-target-<?php echo intval($item_id); ?>">
                        ...
                        <?php _e('Open link in a new window/tab', 'Lawyer'); ?>
                    </label>
                </p>
                <p class="field-css-classes description description-thin">
                    <label for="edit-menu-item-classes-<?php echo intval($item_id); ?>">
                        <?php _e('CSS Classes (optional)', 'Lawyer'); ?><br />
                ...
                <p class="field-xfn description description-thin">
                    <label for="edit-menu-item-xfn-<?php echo intval($item_id); ?>">
                        <?php _e('Link Relationship (XFN)', 'Lawyer'); ?><br />
                ...
                <p class="field-description description description-wide">
                    <label for="edit-menu-item-description-<?php echo intval($item_id); ?>">
                        ...
                        <span class="description"><?php _e('The description will be displayed in the menu if the current theme supports it.', 'Lawyer'); ?></span>
                    </label>
                </p>        
                <?php
                ...
                <p class="field-custom description description-wide custom_onof">
                    <label class="pbwp-checkbox" for="edit-menu-item-megamenu-<?php echo intval($item_id); ?>">
                        <?php _e('Active Mega Menu', 'Lawyer'); ?><br />
        ...
        <?php _e('hide text', 'Lawyer'); ?><br />
                        <input type="hidden" value="off" name="cs_title_switch">
                        <input type="checkbox" id="edit-menu-item-text-<?php echo intval($item_id); ?>" class="widefat code edit-menu-item-custom" name="menu-item-text[<?php echo intval($item_id); ?>]" <?php if (esc_attr($item->text) == 'on') {
            echo 'checked="checked"';
        } ?>  />
                        <span class="pbwp-box"></span>
                    </label>
        ...
        <?php _e('disable link', 'Lawyer'); ?><br />
                        <input type="hidden" value="off" name="cs_title_switch">
                        <input type="checkbox" id="edit-menu-item-link-<?php echo intval($item_id); ?>" class="widefat code edit-menu-item-custom" name="menu-item-link[<?php echo intval($item_id); ?>]" <?php if (esc_attr($item->link) == 'on') {
            echo 'checked="checked"';
        } ?>  />
                        <span class="pbwp-box"></span>
                    </label>
                ...
                <div class="menu-item-actions description-wide submitbox">
                    <?php if ('custom' != $item->type && $original_title !== false) : ?>
                        ...
                        <?php printf(__('Original: %s', 'Lawyer'), '<a href="' . esc_url($item->url) . '">' . esc_html($original_title) . '</a>'); ?>
                        </p>
                    <?php endif; ?>
                    ...
                    ?>"><?php _e('Remove', 'Lawyer'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo intval($item_id); ?>" href="<?php echo esc_url(add_query_arg(array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg($removed_args, urlencode(admin_url('nav-menus.php')))));
                    ?>#menu-item-settings-<?php echo intval($item_id); ?>"><?php _e('Cancel', 'Lawyer'); ?></a>
                </div>
