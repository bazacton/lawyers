...
if (!function_exists('cs_demo_importer')) {
    ...
    function cs_demo_importer() {
        ...
        if (isset($_REQUEST['demo']) && $_REQUEST['demo'] == 'demo-data') {
            ...
            if ($cs_demoimport_error) {
                ...
                if (!is_file(get_template_directory() . '/include/theme-components/cs-importer/demo.xml')) {
                    ...
                    echo '<p><strong>' . __('Sorry, there has been an error.', 'Lawyer') . '</strong><br />';

                    echo __('The file does not exist, please try again.', 'Lawyer') . '</p>';
                } else {
                    ...
                    if ( ! get_option( 'cs_import_demo' ) ) {
                        ...
                        if ($page == 'home-two') {
                            ...
                            $term_exists = term_exists('header-left-menu', 'nav_menu');
                            if (!$term_exists) {
                                $wpdb->insert(
                                        $wpdb->terms, array(
                                    'name' => 'Header Left Menu',
                                    'slug' => 'header-left-menu',
                                    'term_group' => 0
                                        ), array(
                                    '%s',
                                    '%s',
                                ...
                                $insert_id = $wpdb->insert_id;
                                $theme_mod_val['header-left-menu'] = $insert_id;
                                $wpdb->insert(
                                        $wpdb->term_taxonomy, array(
                                    'term_id' => $insert_id,
                                    'taxonomy' => 'nav_menu',
                                ...
                                $theme_mod_val['header-left-menu'] = $term_exists['term_id'];
                            }
                            // header right menu
                            $term_exists = term_exists('header-right-menu', 'nav_menu');
                            if (!$term_exists) {
                                $wpdb->insert(
                                        $wpdb->terms, array(
                                ...
                                $insert_id = $wpdb->insert_id;
                                $theme_mod_val['header-right-menu'] = $insert_id;
                                $wpdb->insert(
                                        $wpdb->term_taxonomy, array(
                                    'term_id' => $insert_id,
                                    'taxonomy' => 'nav_menu',
                                ...
                                $theme_mod_val['header-right-menu'] = $term_exists['term_id'];
                            }
        ...
        <div class="cs-demo-data">
            ...
            <h2><?php _e('Import Demo Data', 'Lawyer'); ?></h2>



            <div class="inn-text">

                <p><?php _e('Importing demo data helps to build site like the demo site by all means. It is the quickest way to setup theme. Following things happen when dummy data is imported;', 'Lawyer'); ?></p>

                <ul class="import-data">

                    <li>&#8226;<?php _e('All wordpress settings will remain same and intact.', 'Lawyer'); ?> </li>

                    <li>&#8226; <?php _e('Posts, pages and dummy images shown in demo will be imported.', 'Lawyer'); ?></li>

                    <li>&#8226;<?php _e('Only dummy images will be imported as all demo images have copy right restriction.', 'Lawyer'); ?> </li>

                    <li>&#8226;<?php _e('No existing posts, pages, categories, custom post types or any other data will be deleted or modified.', 'Lawyer'); ?> </li>

                    <li>&#8226;<?php _e('To proceed, please click "Import Demo Data" and wait for a while.', 'Lawyer'); ?> </li>
...
function cs_update_themeoptions($page = '', $formdata = '', $theme_option = '') {
    if (isset($page->ID)) {
        ...
        echo '<div class="updated"> <h4>' . __('Page not exist', 'Lawyer') . '</h4></div>';
    }
}
 }
