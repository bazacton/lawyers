...
if (!function_exists('cs_logo')) {
    ...
    function cs_logo() {
        global $cs_theme_options;
        $logo = isset($cs_theme_options['cs_custom_logo']) && $cs_theme_options['cs_custom_logo'] ? $cs_theme_options['cs_custom_logo'] : get_template_directory_uri() . '/assets/images/logo-header.jpg';
        ?>
...
if (!function_exists('cs_header_strip')) {
    ...
    function cs_header_strip($container = 'on') {
        global $cs_theme_options;
        if (isset($cs_theme_options['cs_header_options'])) {
        ...
        if (isset($cs_theme_options['cs_wpml_switch'])) {
            $cs_wpml_switch = $cs_theme_options['cs_wpml_switch'];
        } else {
            $cs_wpml_switch = '';
        }
        $cs_header_strip_tagline_text = htmlspecialchars_decode($cs_theme_options['cs_header_strip_tagline_text']);
        if ($cs_header_strip_tagline_text == 'on' || $cs_top_menu_switch == 'on' || $cs_socail_icon_switch == 'on') {
            ?>
            <!-- Top Strip -->
                    ...
                    <?php if ($container == 'on') { ?>
                        ...
                        <aside class="left-side">
                            <?php if (isset($cs_header_strip_tagline_text) and $cs_header_strip_tagline_text <> '') { ?>
                                <?php echo '<p>' . do_shortcode($cs_header_strip_tagline_text) . '</p>'; ?>
                                <?php
                            }
                            ?>
                        </aside>
    ...
    function cs_search($search_class = 'cs-searchv2') {
        ...
        <div class="search-sec <?php echo cs_allow_special_char($search_class); ?>">
            ...
            <form id="searchform" method="get" action="<?php echo home_url() ?>"  role="search">
                <input type="text" name="s" id="searchinput" value="<?php _e('Enter your search', 'Lawyer'); ?>" onblur="if (this.value == '') {
                            this.value = '<?php _e('Enter your search', 'Lawyer'); ?>';
                        }" onfocus="if (this.value == '<?php _e('Enter your search', 'Lawyer'); ?>') {
                                    this.value = '';
                                }"  >
                <label>
...
if (!function_exists('cs_contribute_now')) {
    ...
    function cs_contribute_now() {
        ...
        if (isset($cs_theme_options['cs_contribute_now']) and $cs_theme_options['cs_contribute_now'] == 'on' and $cs_contribute_now_link <> '') {
            echo '<a class="btn-style1" href="' . $cs_contribute_now_link . '"><i class="icon-database"></i>' . __('Contribute Now', 'Lawyer') . '</a>';
        }
    }
...
if (!function_exists('cs_get_headers')) {
    ...
    function cs_get_headers() {
        ...
        $cs_header_options_class = 'header_1';
        if ($cs_header_options == 'Header 1') {
            $cs_header_options_class = 'header_1';
        } else {
            if ($cs_header_options == 'Header 2') {
                $cs_header_options_class = 'header_2';
            }
        }
        ...
        if (isset($cs_header_options) and $cs_header_options == 'header_1') {
            ?>
            <!-- Header Start -->
            <header id="main-header" class="<?php echo cs_allow_special_char($cs_header_options_class); ?>">
                <!--//  TOPBAAR //-->
            <?php cs_header_strip(); ?>
                <!-- Main Header -->
                ...
                <div class="main-navbar">
                    <div class="container">
            <?php cs_main_navigation('header-left-menu', '') ?>

                        <div class="logo"><?php cs_logo(); ?></div>

            <?php
            cs_main_navigation('header-right-menu', '');

            cs_main_navigation('mobile-nav', 'mobile-nav');
            ...
            <!-- Header End --> 
            <!-- Header 1 End -->
        <?php } else if (isset($cs_header_options) and $cs_header_options == 'header_2') {
            ?>
            <header id="main-header" class="<?php echo cs_allow_special_char($cs_header_options_class); ?>">
...
if (!function_exists('cs_header_main_navigation')) {
    ...
    function cs_header_main_navigation($nav = '') {
        ...
        if (empty($cs_xmlObject->page_custom_menu))
            $page_custom_menu = "";
        else
            $page_custom_menu = $cs_xmlObject->page_custom_menu;
...
if (!function_exists('cs_subheader_style')) {
    ...
    function cs_subheader_style($post_ID = '') {
        ...
        if (is_page()) {
            $meta_element = 'cs_page_builder';
        } else if (is_single() && $post_type == 'team') {
            $meta_element = 'team';
        } else if (is_single() && $post_type == 'casestudy') {
            $meta_element = 'casestudy';
        } else if (is_single() && $post_type == 'practice') {
            $meta_element = 'practice';
        } else if (is_single() && $post_type != 'post') {
            $meta_element = 'dynamic_cusotm_post';
        } else {
            $meta_element = 'post';
        ...
        if (isset($cs_xmlObject->header_banner_style) && $cs_xmlObject->header_banner_style == 'no-header') {
            ...
            cs_shortcode_map();
        } else if (isset($cs_xmlObject->header_banner_style) && $cs_xmlObject->header_banner_style == 'default_header') {
            cs_default_page_header();
...
if (!function_exists('cs_default_page_header')) {

    function cs_default_page_header($post_ID = '') {
        global $post, $wp_query, $cs_theme_options, $cs_xmlObject;
        ...
        if (isset($cs_theme_options['subheader_padding_switch']) && $cs_theme_options['subheader_padding_switch'] == 'custom') {
            if (empty($cs_theme_options['cs_sh_paddingtop']))
                $cs_sh_paddingtop = "";
            else
                $cs_sh_paddingtop = 'padding-top:' . $cs_theme_options['cs_sh_paddingtop'] . 'px;';
            if (empty($cs_theme_options['cs_sh_paddingbottom']))
                $cs_sh_paddingbottom = "";
            else
                $cs_sh_paddingbottom = 'padding-bottom:' . $cs_theme_options['cs_sh_paddingbottom'] . 'px;';
        }
        ...
        if ($isDeafultSubHeader == 'true') {
            ...
            $page_subheader_color = $cs_theme_options['cs_sub_header_bg_color'];
            ...
            if (isset($cs_xmlObject->page_subheader_no_image) && $cs_xmlObject->page_subheader_no_image != '') {
                if (empty($cs_xmlObject->header_banner_image))
                    ...
                    $header_banner_image = $cs_xmlObject->header_banner_image;
                if (empty($cs_xmlObject->page_subheader_parallax))
                    $page_subheader_parallax = "";
                else
                    $page_subheader_parallax = $cs_xmlObject->page_subheader_parallax;
            } else {
            ...
            if (isset($cs_xmlObject->subheader_padding_switch) && $cs_xmlObject->subheader_padding_switch == 'custom') {
                if (empty($cs_xmlObject->subheader_padding_top)) {
                    ...
                    $cs_sh_paddingtop = 'padding-top:' . $cs_xmlObject->subheader_padding_top . 'px;';
                }
                if (empty($cs_xmlObject->subheader_padding_bottom)) {
                    $cs_sh_paddingbottom = "";
                } else {
                    $cs_sh_paddingbottom = 'padding-bottom:' . $cs_xmlObject->subheader_padding_bottom . 'px';
                }
            }
        }
        ...
        if (isset($header_banner_image) && $header_banner_image != '') {
            ...
            if ($page_subheader_parallax == 'on') {
                $header_banner_image = 'url(' . $header_banner_image . ') center top / cover ' . $parallaxStatus . ' !important';
                $subheader_style_elements = 'background: ' . $header_banner_image . ' ' . $page_subheader_color . ' !important;';
            } else {
                $subheader_style_elements = '';
                $header_banner_image = 'url(' . $header_banner_image . ') center top / cover' . $parallaxStatus . ' !important';
                $subheader_style_elements = 'background: ' . $header_banner_image . ';' . $page_subheader_color . ' !important;';
        ...
        if (isset($cs_theme_options['cs_breadcrumbs_switch']) && $cs_theme_options['cs_breadcrumbs_switch'] == 'on') {
...
if (!function_exists('cs_shortcode_slider')) {
    ...
    function cs_shortcode_slider($type = '') {
        ...
        if ($type == 'pages') {
            ...
            if (empty($cs_theme_options['cs_custom_slider']))
                $custom_slider_id = "";
            else
                $custom_slider_id = htmlspecialchars($cs_theme_options['cs_custom_slider']);
        }
...
if (!function_exists('cs_shortcode_map')) {
    ...
    function cs_shortcode_map() {
        ...
        if (empty($cs_xmlObject->custom_map))
            ...
            $custom_map = html_entity_decode($cs_xmlObject->custom_map);
...
if (!function_exists('get_subheader_breadcrumb')) {
    ...
    function get_subheader_breadcrumb($page_tile_align = '') {
        ...
        if (( isset($cs_xmlObject->page_breadcrumbs) && $cs_xmlObject->page_breadcrumbs == 'on' ) || ( isset($cs_theme_options['cs_breadcrumbs_switch']) && $cs_theme_options['cs_breadcrumbs_switch'] == 'on' )) {
            ?>
            <!-- BreadCrumb -->
            <div class="breadcrumbs <?php echo cs_allow_special_char($page_tile_align); ?>">
                <div class="container">
            <?php
            if (is_author() || is_search() || is_archive() || is_category()) {
                ...
                if (isset($page_header_style) and $page_header_style == 'default_header') {
                    ...
                    ?>
                            ...
                            if (isset($page_bg_image) && $page_bg_image <> '') {
                                ...
                                <style scoped>
                                    .breadcrumb-sec {
                                        background:url('<?php echo cs_allow_special_char($page_bg_image); ?>') !important;
                                    }	
                                    .breadcrumb-sec, .breadcrumb ul li a,.breadcrumb ul li.active,.breadcrumb ul li:first-child:after {
                    ...
                    ?>
                        <?php } else if (isset($page_subheader_text_color) && $page_subheader_text_color != '') {
                            ?>
                            <style scoped>
                                .breadcrumb-sec, .breadcrumb ul li a,.breadcrumb ul li.active,.breadcrumb ul li:first-child:after {
                                    color : <?php echo cs_allow_special_char($page_subheader_text_color); ?> !important;
