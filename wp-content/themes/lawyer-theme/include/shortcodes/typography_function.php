...
if (!function_exists('cs_column_shortocde')) {
    ...
    function cs_column_shortocde($atts, $content = "") {
        ...
        return '<div class="animated ' . $cs_column_animation . ' ' . $cs_column_class . ' ' . $column_class . '"' . $cs_column_class_id . '>' . $section_title . ' ' . $html . '</div>';
    }
...
if (!function_exists('cs_divider_shortcode')) {
    ...
    function cs_divider_shortcode($atts) {
        ...
        if ($divider_backtotop == 'yes') {
            $backtotop = '<span class="backtotop"><a class="btn-back-top btnnext" href="#"><i class="fa-angle-up"></i></a></span>';
        }
...
if (!function_exists('cs_quote_shortcode')) {
    ...
    function cs_quote_shortcode($atts, $content = null) {
        ...
        $html .= '<blockquote class="cs-qoute ' . $cs_quote_class . ' ' . $quote_align . ' ' . $cs_quote_animation . '" id="' . $cs_quote_class . '" style="animation-duration: ' . $cs_quote_animation_duration . 's; color:' . $quote_text_color . '"><span>' . do_shortcode($content) . $author_name . '</span></blockquote>';
...
if (!function_exists('cs_contactus_shortcode')) {
    ...
    function cs_contactus_shortcode($atts, $content = "") {
        ...
        if (trim($cs_contactus_view) == 'plain') {
            $view_class = 'cs-plain-form';
        } else {
            $view_class = 'cs-classic-form';
        }
        ?>
        ...
        $html .= '<div class="' . $cs_contact_animation . ' ' . $cs_contact_class . ' ' . $view_class . ' cs_form_styling ">';
        ...
        if (isset($cs_contactus_label) && $cs_contactus_label == 'on') {
            $html .= '<label>' . __('Enter Your Name', 'Lawyer') . '</label>';
        }

        $html .= '<input type="text" name="contact_name" placeholder="' . __('Full Name', 'Lawyer') . '" class="' . $class . '" required /></p>';
        $html .= '<p>';
        if (isset($cs_contactus_label) && $cs_contactus_label == 'on') {
            $html .= '<label>' . __('Enter Your Email Address', 'Lawyer') . '</label>';
        }

        $html .= '<input type="email" name="contact_email" placeholder="' . __('example@example.com', 'Lawyer') . '" class="' . $class . '" required /></p>';

        $html .= '<p>';
        if (isset($cs_contactus_label) && $cs_contactus_label == 'on') {
            $html .= '<label>' . __('Enter Your Phone Number', 'Lawyer') . '</label>';
        }

        $html .= '<input type="text" name="contact_number" placeholder="' . __('Phone Number', 'Lawyer') . '" class="' . $class . '" /></p>';
        $html .= '<p>';
        if (isset($cs_contactus_label) && $cs_contactus_label == 'on') {
            $html .= '<label>' . __('Enter Subject', 'Lawyer') . '</label>';
        }

        $html .= '<input type="text" name="subject" placeholder="' . __('Subject', 'Lawyer') . '" class="' . $class . '" required /></p>';
        $html .= '<p class="textarea_sec">';
        if (isset($cs_contactus_label) && $cs_contactus_label == 'on') {
            $html .= '<label>' . __('Message', 'Lawyer') . '</label>';
        }

        $html .= '<textarea class="' . $class . ' ' . $view_class . '" placeholder="' . __('Message', 'Lawyer') . '" name="contact_msg" required></textarea></p>';
        $html .= '<input type="submit" value="' . __('Submit Now', 'Lawyer') . '" class="custom-btn cs-bg-color" id="submit_btn' . $cs_email_counter . '">';
        $html .= '</form>';
...
if (!function_exists('cs_message_shortcode')) {
    ...
    function cs_message_shortcode($atts, $content = "") {
        ...
        if (isset($cs_message_type) && $cs_message_type == 'alert') {
            ...
            if (isset($cs_message_icon) && $cs_message_icon <> '') {
                $html .= '<i style="color:' . $cs_icon_color . ';" class="fa-cog"></i>';
            }
...
if (!function_exists('cs_testimonials_shortcode')) {
    ...
    function cs_testimonials_shortcode($atts, $content = null) {
        global $cs_testimonial_class, $column_class, $testimonial_text_color, $section_title;
        $randomid = rand(0, 999);
        //cs_owl_carousel();
        $defaults = array('column_size' => '1/1', 'testimonial_text_color' => '', 'cs_testimonial_text_align' => '', 'cs_testimonial_section_title' => '', 'cs_testimonial_class' => '', 'cs_testimonial_animation' => '', 'cs_testimonial_animation_duration' => '', 'cs_testimonial_view' => '');
        extract(shortcode_atts($defaults, $atts));
        $column_class = cs_custom_column_class($column_size);
        $html = '';
        $section_title = '';
		...
		$view = isset($cs_testimonial_view) ? $cs_testimonial_view: '';
        if ($cs_testimonial_section_title && trim($cs_testimonial_section_title) != '') {
            $section_title = '<div class="main-title"><div class="cs-section-title"><h2>' . $cs_testimonial_section_title . '</h2></div></div>';
        }
        ...
        <script type='text/javascript'>
            jQuery(document).ready(function () {
                jQuery("#cs-testimonial-<?php echo cs_allow_special_char($randomid); ?>").flexslider({
                    ...
                    prevText: "<em class='fa-angle-left'></em>",
                    nextText: "<em class='fa-angle-right'></em>",
        ...
        /* 	if(isset($testimonial_style) && $testimonial_style == 'fancy'){
          ...
          $html .= '
          ' .$section_title. '
          <div id="cs-testimonial-'.cs_allow_special_char($randomid).'" class="flexslider testimonial '.$cs_testimonial_text_align.$testim_clas.'">
        ...
        $html .= '
			...
			<div id="cs-testimonial-' . cs_allow_special_char($randomid) . '" class="flexslider testimonial ' . $cs_testimonial_text_align . $testim_clas . '">
				<div class="flex-viewport">
				 <ul class="' . cs_allow_special_char($view) . '">
...
if (!function_exists('cs_testimonial_item')) {
    ...
    function cs_testimonial_item($atts, $content = null) {
        global $cs_testimonial_class, $column_class, $testimonial_text_color;
        ...
        /* 	if(isset($testimonial_style) && $testimonial_style == 'fancy'){
          ...
          <div class="ts-author">
          '.$figure.'
          <h4 class="cs-author" '.$tc_color.'>'.$testimonial_author.'<br>
