        ...
        <div class="row">        
            ...
            <div class="<?php echo esc_attr($cs_layout); ?>">
                <div class="page-no-search">
                    <?php
                    if (have_posts()) :
                        echo '<h1>' . __('Showing result for "' . get_search_query() . '"', 'Lawyer') . '</h1>';
                        echo '<div class="relevant-search"><ul>';
                        while (have_posts()) : the_post();
                            ?>	
                            <li>
                                <?php
                                if (is_sticky()) {
                                    echo '<span>' . __('Featured', 'Lawyer') . '</span>';
                                }
                                echo '<h5>' . date_i18n(get_option('date_format'), strtotime(get_the_date()));
                                ?>, <?php echo cs_get_the_excerpt('51', false); ?></h5>
